...
```haskell
someLookup :: PrimMonad m => DimV VU.MVector (PrimState m) '[5,3,3] Int -> m Int
...
Not really a problem but it's nice to mix mutable lenses with a state monad to hold the top-level values. This state monad could be interpreted as storage for local variables. Adding mutable lens operators that work with state monads adds even more code duplication, though.
